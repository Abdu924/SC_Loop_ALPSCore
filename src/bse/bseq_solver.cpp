...
void BseqSolver::inverse_bseq() {     
     {
          ...
          // Initizalize lattice_chi objects (per proc and global)
          if (world_rank_ == 0)
          {
          ...
          // Invert the BSE equation for all qs, per processor (Q-level MPI parallel)
     ...
     // gather the results from all procs (q-level MPI parallelization) into the 0-rank process
     if (world_rank_ == 0) {
...
void BseqSolver::read_local_bubble(alps::hdf5::archive &bubble_h5_archive) {
     cout << "entering read_local_bubble" << endl;
     ...
     cout << "read_local_bubble DONE" << endl;
}
